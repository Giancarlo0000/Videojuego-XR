...
public class Dron : MonoBehaviour
{
    ...
    [SerializeField] private float BulletFrequency = 3f;
    ...
    private void PatrolPerimeter()
    {
        if (_spawner == null)
        {
            Debug.LogError("Error en PatrolPerimeter: Falta el spawner");
            return;
        }
    ...
    private void StartShooting()
    {
        ...
        {
            _isShooting = true;
            InvokeRepeating(nameof(InstantiateProjectiles), BulletFrequency, BulletFrequency);
        }
    }
    ...
    public void TakeDamage(float damage)
    {
        ...
        if (Life <= 0)
        {
            ...
            GameObject explosion = ObjectPooling.Instance.SpawnFromPool("ExplosionParticles", transform.position, Quaternion.identity);
            if (explosion != null)
            {
                ...
                {
                    particleSystem.Play();
                    StartCoroutine(DisableAfterTime(explosion, particleSystem.main.duration));
                }
            }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            float damageToPlayer = Life / 2f;
            GameManager.Instance.healthPlayer -= damageToPlayer;
            gameObject.SetActive(false);
        }
    }
}
